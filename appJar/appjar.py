...
class Enum(object):
    ...
    def get(self, name):
        try:
            return self.widgets.index(name)
        except:
    ...
    def __setattr__(self, name, value):
        if self.__initialized:
            raise Exception("Unable to change Widget store")
        else:
...
###############################################
    ...
    def SET_LOCATION(x, y=None, ignoreSettings=None, win=None, up=0):
        ...
        if isinstance(x, UNIVERSAL_STRING) and x.lower() in ['c', 'center', 'centre']:
            x = dims["x"]
        if isinstance(y, UNIVERSAL_STRING) and y.lower() in ['c', 'center', 'centre']:
            y = dims["y"]
        ...
        if x < 0 or x > dims['s_width']:
            x = dims['x']
        if y < 0 or y > dims['s_height']:
            y = dims['y']
    ...
    def GET_DIMS(container):
        ...
        dims["outerFrameWidth"] = 0 if container.winfo_x() == 0 else container.winfo_rootx() - \
            container.winfo_x()
        dims["titleBarHeight"] = 0 if container.winfo_rooty(
        ) == 0 else container.winfo_rooty() - container.winfo_y()
...
#####################################
    def __init__(
        ...
        if logLevel is not None:
            gui.setLogLevel(logLevel)

        if handleArgs:
            if args.l:
                self._language = args.l
            if args.ttk:
                useTtk = True
    ...
    def _handleArgs(self):
        ...
        parser.add_argument("-v", "--version", action="version",
                            version=gui.SHOW_VERSION(), help="show version information and exit")
        logGroup = parser.add_mutually_exclusive_group()
        logGroup.add_argument("-c", action="store_const", const=True,
                              help="only log CRITICAL messages")
        logGroup.add_argument("-e", action="store_const", const=True,
                              help="log ERROR messages and above")
        logGroup.add_argument("-w", action="store_const", const=True,
                              help="log WARNING messages and above")
        logGroup.add_argument("-i", action="store_const", const=True,
                              help="log INFO messages and above")
        logGroup.add_argument("-d", action="store_const", const=True,
                              help="log DEBUG messages and above")
        logGroup.add_argument("-t", action="store_const", const=True,
                              help="log TRACE messages and above")
        ...
        parser.add_argument("-s", metavar="SETTINGS", const=True, nargs="?",
                            help="load settings, from an optional file name")
        parser.add_argument("--ttk", metavar="THEME", const=True, nargs="?",
                            help="enable ttk, with an optional theme")
    ...
    def _configBg(self, container):
        ...
        if not self.ttkFlag:
            self.bgLabel = BgLabel(container, anchor=CENTER, font=self._getContainerProperty(
                'labelFont'), background=self._getContainerBg())
        else:
    ...
    def setTtkTheme(self, theme=None):
        ...
        if theme is not None:
            try:
                ...
                self._loadTtkThemes()
                if not ThemedStyle:
                    self.error("ttk theme: %s unavailable. Try one of: %s",
                               theme, str(self.ttkStyle.theme_names()))
                else:
                    self.ttkStyle.set_theme(theme)

        # set up our ttk styles
        self.ttkStyle.configure("DefaultText.TEntry", foreground="grey")
        self.ttkStyle.configure("ValidationEntryValid.TEntry", foreground="#4CC417",
                                highlightbackground="#4CC417", highlightcolor="#4CC417", highlightthickness='20')
        self.ttkStyle.configure("ValidationEntryInvalid.TEntry", foreground="#FF0000",
                                highlightbackground="#FF0000", highlightcolor="#FF0000", highlightthickness='20')
        self.ttkStyle.configure("ValidationEntryWait.TEntry", foreground="#000000",
                                highlightbackground="#000000", highlightcolor="#000000", highlightthickness='20')
...
###############################################################
    ...
    def _loadExternalDnd(self):
        ...
        if EXTERNAL_DND is None:
            try:
                tkdndlib = os.path.join(os.path.dirname(
                    os.path.abspath(__file__)), "lib", "tkdnd2.8")
...
#####################################
    ...
    def _registerInternalDragSource(self, kind, title, widget, function=None):
        ...
        if INTERNAL_DND is not False:
            try:
                widget.bind('<ButtonPress>', lambda e: self._startInternalDrag(
                    e, title, name, widget))
                ...
                for k, v in self.widgetManager.group(WIDGET_NAMES.Image).items():
                    if v == event.widget:
                        try:
                            ...
                            self.errorBox("Error loading image",
                                          "Unable to load image: " + str(event.data))
                        return
                ...
                for (key, val) in self.configParser.items(section):
                    ...
                    if len(keys) != 2:
                        ...
                        if keys[1] not in ["prevButton", "nextButton", "title"]:
                            self.warn("Invalid PAGEDWINDOW label: %s for PAGEDWINDOW: %s",
                                      keys[1], keys[0])
                        else:
                            try:
    ...
    def logMessage(msg, level, *args):
        ...
        if frames[1][3] in ("exception", "critical", "error", "warn", "debug", "trace", "info"):
            ...
            # appJar logging
                ...
                if callFrame != "":
                    msg = "["+callFrame + "->" + \
                        str(frames[2][2]) + "/"+str(frames[2][3])+"]: "+str(msg)
                else:
...
##############################################################
    ...
    def go(self, language=None, startWindow=None):
        ...
        if self.GET_PLATFORM() == self.MAC:
            self.topLevel.createcommand('tk::mac::ReopenApplication', self._macReveal)
    ...
    def getSetting(self, name, default=None):
        ...
        try:
            return self.externalSettings[name]
        except:
            return default
    ...
    def saveSettings(self, fileName="appJar.ini"):
        ...
        for k, v in self.widgetManager.group(WIDGET_NAMES.ToggleFrame).items():
            ...
            if "TOGGLES" not in settings.sections():
        ...
        for k, v in self.widgetManager.group(WIDGET_NAMES.TabbedFrame).items():
            ...
            if "TABS" not in settings.sections():
        ...
        for k, v in self.widgetManager.group(WIDGET_NAMES.PagedWindow).items():
            ...
            if "PAGES" not in settings.sections():
                settings.add_section("PAGES")
            settings.set("PAGES", k, str(v.getPageNumber()))

        for k, v in self.widgetManager.group(WIDGET_NAMES.SubWindow).items():
            if "SUBWINDOWS" not in settings.sections():
                settings.add_section("SUBWINDOWS")
            if v.shown:
                v.update()
                ...
                gui.trace("Saving subWindow %s: geom=%s, state=%s, minsize=%s",
                          k, v.geometry(), v.state(), ms)
            else:
                settings.set("SUBWINDOWS", k, "False")
                gui.trace("Skipping subwindow: %s", k)

        for k, v in self.externalSettings.items():
            if "EXTERNAL" not in settings.sections():
...
#####################################
    ...
    def bindKey(self, key, func, replace=False):
        ...
        try:
            ...
            raise ItemLookupError('Unable to bind key ' +
                                  binding.displayName + ' - binding already exists')
...
#####################################
    ...
    def setMinSize(self, container=None, size=None):
        ...
        if size is None:
            size = (gui.GET_DIMS(container)["r_width"], gui.GET_DIMS(container)["r_height"])
        container.ms = size
...
#            val=os.system('''/usr/bin/osascript -e 'tell app "Finder" to set frontmost of process "''' + PY_NAME + '''" to true' ''')
    ...
    def setFullscreen(self, title=None):
        ...
        if not container.isFullscreen:
            ...
            container.escapeBindId = container.bind(
                '<Escape>', self.MAKE_FUNC(self.exitFullscreen, container), "+")
    ...
    def configure(self, **kwargs):
        ...
        if inPadding is not None:
            self.inPadding = inPadding
        if guiPadding is not None:
            self.guiPadding = guiPadding
        ...
        if buttonFont is not None:
            self.buttonFont = buttonFont
        if inputFont is not None:
            self.inputFont = inputFont
        if statusFont is not None:
            self.statusFont = statusFont
        if ttkTheme is not None:
        ...
        if fastStop is not None:
            self.fastStop = fastStop
        if enterKey is not None:
    ...
    def _rightClick(self, event, menu="EDIT"):
        ...
        if menu == "EDIT":
            if self._prepareCopyAndPasteMenu(event):
                ...
                self.widgetManager.get(WIDGET_NAMES.Menu, menu).post(
...
#####################################
    ...
    def configureWidget(self, kind, name, option, value, key=None, deprecated=False):
        ...
        if deprecated:
            self.warn("Deprecated config function (%s) used for %s -> %s use %s deprecated",
                      option, WIDGET_NAMES.name(kind), name, deprecated)
    ...
    def _validateFunctionList(self, functions, mode):
        ...
        if len(functions) != 2:
            raise Exception(
                "Invalid arguments, set<widget> %s Function requires 1 or 2 functions to be passed in.", mode)
    ...
    def _bindEvent(self, kind, name, widget, function, eventType, key=None):
        ...
        if kind == WIDGET_NAMES.Scale:
            ...
            if widget.kind == "ticks":
                vals = self.widgetManager.get(
                    WIDGET_NAMES.TickOptionBox, name, group=WidgetManager.VARS)
                for o in vals:
                    cmd = self.MAKE_FUNC(function, name)
    ...
    def hideWidgetType(self, kind, name, collapse=False):
        ...
        for item in items:
            if self._widgetHasContainer(kind, item):
                ...
                try:
                    self.widgetManager.get(WIDGET_NAMES.FrameLabel, name).hidden = True
                except:
                    pass
            else:
...
#                            widget = rb
        ...
        for item in items:
            if self._widgetHasContainer(kind, item):
                ...
                if hasattr(widget, "inContainer") and widget.inContainer:
                    ...
                    widget = widget.master
                try:
                    self.widgetManager.get(WIDGET_NAMES.FrameLabel, name).hidden = False
                except:
                    pass
            else:
                msg = "Showing widget"
    ...
    def removeAllWidgets(self, current=False, sub=False):
        if current:
            ...
            containerData = self._prepContainer(
                containerData["title"], containerData["type"], containerData["container"], 0, 1)
...
#####################################
    ...
    def _positionWidget(self, widget, row, column=0, colspan=0, rowspan=0, sticky=W+E, updateBg=True):
        ...
        if colspan != 0:
            ...
            params["columnspan"] = cColspan

        if rowspan != 0:
            params["rowspan"] = rowspan
        elif cRowspan != 0:
            params["rowspan"] = cRowspan
...
#####################################
    ...
    def openFrame(self, title):
        try:
            return self._openContainer(WIDGET_NAMES.Frame, title)
        except:
            return self._openContainer(WIDGET_NAMES.SubFrame, title)
    ...
    def startContainer(self, fType, title, row=None, column=0, colspan=0, rowspan=0, sticky=None, name=None):
        ...
        if fType == WIDGET_NAMES.LabelFrame:
            ...
            tabbedFrame = self._tabbedFrameMaker(
                self.getContainer(), self.ttkFlag, font=self._getContainerProperty('labelFont'))
            if not self.ttkFlag:
...
#            tabbedFrame.isContainer = True
            ...
            if name not in ["horizontal", "vertical", ""]:
                gui.warn(
                    "ScrollPane %s: Invalid value for disabled, must be one of 'horizontal' or 'vertical'", title)
            ...
            pagedWindow = PagedWindow(self.getContainer(), title=title, bg=self._getContainerBg(
            ), width=200, height=400, buttonFont=self._getContainerProperty('buttonFont'), titleFont=self._getContainerProperty('labelFont'))
            # bind events
...
#####################################
    ...
    def _tabbedFrameMaker(self, master, useTtk=False, **kwargs):
        ...
        class TabText(labelBase, object):
            ...
            def display(self, fill=False, beforeTab=None, afterTab=None):
                ...
                if not self.hidden:
                    if fill:
                        self.pack(side=LEFT, ipady=4, ipadx=4, expand=True,
                                  fill=BOTH, before=beforeTab, after=afterTab)
                    else:
                        self.pack(side=LEFT, ipady=4, ipadx=4, before=beforeTab, after=afterTab)
        ...
        class TabbedFrame(frameBase, object):
            def __init__(self, master, fill=False, changeOnFocus=True, font=None, **kwargs):
                ...
                if gui.GET_PLATFORM() == gui.MAC:
                    self.inactiveCursor = "pointinghand"
                elif gui.GET_PLATFORM() in [gui.WINDOWS, gui.LINUX]:
                    self.inactiveCursor = "hand2"
                ...
                if useTtk:
                    ...
                    self.ttkStyle.configure(
                        "ActiveTab.TLabel", foreground=self.activeFg, background=self.activeBg)
                    self.ttkStyle.configure("InactiveTab.TLabel",
                                            foreground=self.inactiveFg, background=self.inactiveBg)
                    self.ttkStyle.configure("DisabledTab.TLabel",
                                            foreground=self.disabledFg, background=self.disabledBg)
            ...
            def addTab(self, text, **kwargs):
                ...
                tab = TabText(self.tabContainer, text=text,
                              func=self.changeTab, font=self.tabFont, **kwargs)
            ...
            def changeTab(self, tabName, callFunction=True):
                ...
                self.selectedTab = tabName
                self._configTabs()
                if self.changeEvent is not None and callFunction:
                    self.changeEvent(tabName)
            ...
            def _configTabs(self):
                for key in list(self.widgetStore.keys()):
                    if self.widgetStore[key][0].disabled:
                        if not useTtk:
                            self.widgetStore[key][0].config(
                                bg=self.disabledBg, fg=self.disabledFg, cursor="")
                        else:
                        ...
                        if key == self.selectedTab:
                            if not useTtk:
                                self.widgetStore[key][0].config(
                                    bg=self.widgetStore[key][1].cget('bg'), fg=self.activeFg, cursor="")
                            else:
                                self.widgetStore[key][0].config(
                                    style="SelectedTab.TLabel", cursor="")
                            self.widgetStore[key][1].lift()
                        else:
                            if not useTtk:
                                self.widgetStore[key][0].config(
                                    bg=self.inactiveBg, fg=self.inactiveFg, cursor=self.inactiveCursor)
                            else:
                                self.widgetStore[key][0].config(
                                    style="InactiveTab.TLabel", cursor=self.inactiveCursor)
    ...
    def tabbedFrame(self, title, row=None, column=0, colspan=0, rowspan=0, sticky="NSEW", **kwargs):
        ...
        if command is not None:
            self.setTabbedFrameChangeCommand(title, command)
        self.configure(**kwargs)
        try:
            yield tabs
        finally:
        ...
        if tabTitle is None:
            try:
                ...
                if self._getContainerProperty('type') != WIDGET_NAMES.TabbedFrame:
                    raise Exception("Can't open a Tab in the current container: ",
                                    self._getContainerProperty('type'))
                else:
    ...
    def startTab(self, title, beforeTab=None, afterTab=None):
        ...
        if self._getContainerProperty('type') == WIDGET_NAMES.Tab:
            ...
            raise Exception("Can't add a Tab to the current container: ",
                            self._getContainerProperty('type'))
...
#####################################
    ...
    def table(self, title, value=None, *args, **kwargs):
        ...
        try:
            self.widgetManager.verify(widgKind, title)
        except:  # widget exists
            if value is not None:
                self.replaceAllTableRows(title, value)
        ...
        if edit is not None:
            self.setTableEditFunction(title, edit)
    ...
    def getTableColumn(self, title, columnNumber):
        grid = self.widgetManager.get(WIDGET_NAMES.Table, title)
        return grid.getRow(columnNumber)
...
#####################################
    ...
    def startPanedFrame(self, title, row=None, column=0, colspan=0, rowspan=0, sticky="NSEW"):
        p = self.startContainer(WIDGET_NAMES.PanedFrame, title, row,
                                column, colspan, rowspan, sticky)
        return p
...
#####################################
    ...
    def startLabelFrame(self, title, row=None, column=0, colspan=0, rowspan=0, sticky=W, hideTitle=False, label=None, name=None):
        ...
        if hideTitle:
            name = ''
        lf = self.startContainer(WIDGET_NAMES.LabelFrame, title, row,
                                 column, colspan, rowspan, sticky, name)
        return lf
        ...
        if title is None:  # new subFrame
            ...
            try:
                if frameNumber is not None:
                    fr = self.openSubFrame(title, frameNumber)
                else:
...
#####################################
    ...
    def frameStack(self, title, row=None, column=0, colspan=0, rowspan=0, sticky="NSEW", **kwargs):
        ...
        try:
            fr = self.startFrameStack(title, row, column, colspan, rowspan,
                                      sticky, change=change, start=start)
        except ItemLookupError:
    ...
    def startFrameStack(self, title, row=None, column=0, colspan=0, rowspan=0, sticky="news", change=None, start=-1):
        fs = self.startContainer(WIDGET_NAMES.FrameStack, title, row,
                                 column, colspan, rowspan, sticky)
            ...
            if widgType not in [WIDGET_NAMES.Tab, WIDGET_NAMES.Page]:
                ...
                if widgType == WIDGET_NAMES.FrameLabel:
                    ...
                    if not self.widgetManager.destroyWidget(WIDGET_NAMES.Label, widget):
                        self.warn(
                            "Unable to destroy %s, during cleanse - destroy returned False", WIDGET_NAMES.Label)
            else:
...
#        elif widgType in ('CanvasDnd', 'ValidationLabel', 'TabBorder', 'TabContainer', 'TabText', 'BgLabel') or hasattr(widget, 'SKIP_CLEANSE'):
            ...
            self.warn("Unable to destroy %s, during cleanse - NO APPJAR TYPE",
                      gui.GET_WIDGET_CLASS(widget))
...
#####################################
    ...
    def checkBox(self, title, value=None, *args, **kwargs):
        ...
        try:
            ...
            if value is not None:
                self.setCheckBox(title, ticked=value, callFunction=callFunction)
            check = self.getCheckBox(title)
        else:  # new widget
            kwargs = self._parsePos(kwargs.pop("pos", []), kwargs)
            check = self._checkBoxMaker(title, *args, **kwargs)
            if value is not None:
                self.setCheckBox(title, value)
        if text is not None:
            self.setCheckBoxText(title, text)
...
#####################################
    ...
    def scale(self, title, *args, **kwargs):
        ...
        if _range is not None:
            self.setScaleRange(title, _range[0], _range[1])
        if vert:
            self.setScaleVertical(title)
        if increment is not None:
            self.setScaleIncrement(title, increment)
        if interval is not None:
            self.showScaleIntervals(title, interval)
        if show:
            self.showScaleValue(title)
        if value is not None:
            self.setScale(title, value, callFunction)
    ...
    def _buildScale(self, title, frame):
        ...
        if not self.ttkFlag:
            scale = self._makeAjScale()(frame, increment=10, variable=var, repeatinterval=10,
                                        orient=HORIZONTAL, font=self._getContainerProperty('inputFont'))
    ...
    def _scaleMaker(self, title, label, row=None, column=0, colspan=0, rowspan=0, **kwargs):
        if label:
            return self.addLabelScale(title, row, column, colspan, rowspan, label)
        else:
    ...
    def clearAllScales(self, callFunction=False):
        for sc in self.widgetManager.group(WIDGET_NAMES.Scale):
            self.setScale(sc, self.widgetManager.get(WIDGET_NAMES.Scale,
                          sc).cget("from"), callFunction=callFunction)
...
#####################################
    ...
    def optionBox(self, title, value=None, *args, **kwargs):
        ...
        try:
            ...
            if mode == "select":
                if value is not None:
                    self.setOptionBox(title, index=value, value=True,
                                      callFunction=callFunction, override=override)
                else:
                    gui.error("No item specified to select in optionBox: %s", title)
            elif mode == "deselect":
                if value is not None:
                    self.setOptionBox(title, index=value, value=False,
                                      callFunction=callFunction, override=override)
                else:
                ...
                if value is not None:
                    gui.error("No value should be specified wen clearing optionBox: %s", title)
                else:
                    self.clearOptionBox(title, callFunction=callFunction)
            elif mode == "rename":
                if value is not None:
                    self.renameOptionBox(title, item=value, newName=newName,
                                         callFunction=callFunction)
                else:
                    gui.error("No item specified to rename in optionBox: %s", title)
            elif mode == "replace":
                if value is not None:
                    self.changeOptionBox(title, options=value, index=index,
                                         callFunction=callFunction)
                else:
                    gui.error("No values specified to replace in optionBox: %s", title)
            elif mode == "delete":
                if value is not None:
                    self.deleteOptionBox(title, index=value)
                else:
                    gui.error("No item specified to delete in optionBox: %s", title)
            elif mode == "get":
                pass
            else:
            ...
            if kind == "ticks":
                if label:
                    opt = self.addLabelTickOptionBox(
                        title, value, *args, label=label, disabled=disabled, **kwargs)
                else:
                    opt = self.addTickOptionBox(title, value, *args, disabled=disabled, **kwargs)
            else:
                if label:
                    opt = self.addLabelOptionBox(
                        title, value, *args, label=label, disabled=disabled, **kwargs)
                else:
                    opt = self.addOptionBox(title, value, *args, disabled=disabled, **kwargs)
                if selected is not None:
                    self.setOptionBox(title, selected)
...
#            option.config(highlightbackground=self._getContainerBg())
    ...
    def addTickOptionBox(self, title, options, row=None, column=0, colspan=0, rowspan=0, disabled="-", **kwargs):
        ...
        :raises ItemLookupError: if the title is already in use
        """
        tick = self._buildOptionBox(self.getContainer(), title, options,
                                    kind="ticks", disabled=disabled)
    ...
    def getOptionBox(self, title):
        ...
        if box.kind == "ticks":
            val = self.widgetManager.get(WIDGET_NAMES.TickOptionBox,
                                         title, group=WidgetManager.VARS)
            retVal = {}
    ...
    def clearOptionBox(self, title, callFunction=True):
        ...
        if box.kind == "ticks":
            ...
            ticks = self.widgetManager.get(WIDGET_NAMES.TickOptionBox,
                                           title, group=WidgetManager.VARS)
            for k in ticks:
            ...
            if index is None:
                ...
                with PauseCallFunction(callFunction, tick, useVar=False):
                    if value is None:  # then we need to delete it
                        ...
                        self.widgetManager.remove(WIDGET_NAMES.TickOptionBox,
                                                  title, index, group=WidgetManager.VARS)
                        ...
                        ticks = self.widgetManager.get(
                            WIDGET_NAMES.TickOptionBox, title, group=WidgetManager.VARS)
                        ticks[value] = ticks.pop(index)
                        box.options[index_num] = value
                        self.changeOptionBox(title, box.options)
                        for tick in ticks:
                            self.widgetManager.get(WIDGET_NAMES.TickOptionBox, title, group=WidgetManager.VARS)[
                                tick].set(ticks[tick].get())
            else:
                if value is None:
            ...
            if count > 0:
                ...
                if not isinstance(index, int):
                    try:
                        ...
                        if value is None:
                            self.warn(
                                "Unknown option in deleteOptionBox: %s in OptionBox: %s", index, title)
                        else:
                            self.warn(
                                "Unknown option in setOptionBox: %s in OptionBox: %s", index, title)
                        return
                    ...
                    if value is None:  # then we can delete it...
                        ...
                        with PauseCallFunction(callFunction, box):
                            if not box['menu'].invoke(index):
                                if override:
                                    gui.trace(
                                        "Setting OptionBox: %s to disabled option: %s", title, index)
                                    box["menu"].entryconfigure(index, state="normal")
                                    box['menu'].invoke(index)
                                    box["menu"].entryconfigure(index, state="disabled")
                                else:
                                    self.warn(
                                        "Unable to set disabled option: %s in OptionBox %s. Try setting 'override=True'", index, title)
                            else:
                                gui.trace("Invoked item: %s", index)
                    else:
                        gui.trace("Renaming: %s from OptionBox: %s to: %s", index, title, value)
                        pos = box.options.index(self.widgetManager.get(
                            WIDGET_NAMES.OptionBox, title, group=WidgetManager.VARS).get())
                        box.options[index] = value
                        self.changeOptionBox(title, box.options, pos)

            else:
                self.widgetManager.get(WIDGET_NAMES.OptionBox, title,
                                       group=WidgetManager.VARS).set("")
...
#####################################
    ...
    def map(self, title, value=None, *args, **kwargs):
        ...
        if value is not None:
            self.setGoogleMapLocation(title, value)
        if zoom is not None:
            self.setGoogleMapZoom(title, zoom)
        if size is not None:
            self.setGoogleMapSize(title, size)
        if terrain is not None:
            self.setGoogleMapTerrain(title, terrain)
        if proxy is not None:
            self.setGoogleMapProxy(title, proxy)
...
#####################################
    ...
    def spinBox(self, title, value=None, *args, **kwargs):
        ...
        try:
            ...
            if mode == "select":
                if value is not None:
                    self.setSpinBoxPos(title, value, *args, **kwargs)
                else:
                    gui.error("No item specified to select in spinbox: %s", title)
            elif mode == "toggle":
                ...
                if value is not None:
                    self.changeSpinBox(title, vals=value)
                else:
                    gui.error("No values specified to replace in spinbox: %s", title)
            elif mode == "delete":
                gui.error("%s not implemented yet in spinbox: %s", mode, title)
            elif mode == "get":
                pass
            ...
            if endValue is not None:
                if label:
                    spinBox = self.addLabelSpinBoxRange(
                        title, value, endValue, *args, label=label, **kwargs)
                else:
                    spinBox = self.addSpinBoxRange(title, value, endValue, *args, **kwargs)
            else:
                if label:
                    spinBox = self.addLabelSpinBox(title, value, *args, label=label, **kwargs)
                else:
                    spinBox = self.addSpinBox(title, value, *args, **kwargs)

        if selected is not None:
            self.setSpinBoxPos(title, selected)
        if item is not None:
            self.setSpinBox(title, item)
...
#####################################
    ...
    def image(self, title, value=None, *args, **kwargs):
        ...
        if speed is not None:
            self.setAnimationSpeed(title, speed)
        if over is not None:
            self.setImageMouseOver(title, over)
        if submit is not None:
            if _map is not None:
                self.setImageMap(title, submit, _map)
            else:
                self.setImageSubmitFunction(title, submit)
        elif submit is None and _map is not None:
            gui.warn("Must specify a submit function when setting an image map: %s", title)
        if drop is not None:
            self.setImageDropTarget(title, drop)
...
#####################################
    ...
    def radioButton(self, title, name=None, *args, **kwargs):
        ...
        if name is None:
            return self.getRadioButton(title)  # no name = get
        else:
            ident = title + "-" + name
            try:
                self.widgetManager.verify(widgKind, ident)
            except:
            ...
            if selected:
                self.setRadioButton(title, name)
            if change is not None:
                self.setRadioButtonChangeFunction(title, change)
            if kind == "square":
                if self.platform == self.MAC:
    ...
    def clearAllRadioButtons(self, callFunction=False):
        for rb in self.widgetManager.group(WIDGET_NAMES.RadioButton, group=WidgetManager.VARS):
            self.setRadioButton(rb, self.widgetManager.get(
                WIDGET_NAMES.RadioButton, rb, group=WidgetManager.VARS).startVal, callFunction=callFunction)

    def setRadioTick(self, title, tick=True):
        self.warn("Deprecated function (%s) used for %s -> %s use %s instead",
                  'setRadioTick', 'radioButton', title, 'setRadioSquare')
...
#####################################
    ...
    def listBox(self, title, value=None, *args, **kwargs):
        ...
        try:
            ...
            if mode == "select":
                if value is not None:
                    ...
                    gui.error("No item specified to select in listbox: %s", title)
            elif mode == "deselect":
                if value is not None:
                    if isinstance(value, int):
                    ...
                    gui.error("No item specified to deselect in listbox: %s", title)
            elif mode == "toggle":
                ...
                if value is not None:
                    self.updateListBox(title, items=value, callFunction=callFunction)
                else:
                    gui.error("No values specified to replace in listbox: %s", title)
            elif mode == "delete":
                if value is not None:
                    if isinstance(value, int):
                    ...
                    gui.error("No value specified to delete in listbox: %s", title)
            elif mode == "add":
                if value is not None:
                    ...
                    gui.error("No value specified to add in listbox: %s", title)
            elif mode == "get":
                pass
            else:
        ...
        if rows is not None:
            self.setListBoxRows(title, rows)
        if multi:
            self.setListBoxMulti(title)
        if group:
            self.setListBoxGroup(title)
        if selected is not None:
            self.selectListItemAtPos(title, selected, callFunction=False)
    ...
    def selectListItem(self, title, item, callFunction=True):
        ...
        if len(positions) > 1 and lb.cget("selectmode") == EXTENDED:
            ...
            gui.warn(
                "Unable to select multiple items for list: %s. Selecting first item: %s", title, item[0])
    ...
    def removeListItemAtPos(self, title, pos):
        ...
        if pos >= len(items):
            raise Exception("Invalid position: " + str(pos) +
                            " must be between 0 and " + str(len(items)-1))
        lb.delete(pos)
...
#####################################
    ...
    def button(self, title, value=None, *args, **kwargs):
        ...
        try:
            ...
            if image is not None:
                button = self._buttonMaker(title, value, "image", image, *args, **kwargs)
            elif icon is not None:
                button = self._buttonMaker(title, value, "icon", icon, *args, **kwargs)
            elif name is not None:
                button = self._buttonMaker(title, value, "named", name, *args, **kwargs)
            else:
                button = self._buttonMaker(title, value, "button", None, *args, **kwargs)
    ...
    def _buttonMaker(self, title, func, kind, extra=None, row=None, column=0, colspan=0, rowspan=0, *args, **kwargs):
        ...
        if kind == "button":
            return self.addButton(title, func, row, column, colspan, rowspan)
        elif kind == "named":
            return self.addNamedButton(extra, title, func, row, column, colspan, rowspan)
        elif kind == "image":
            return self.addImageButton(title, func, extra, row, column, colspan, rowspan, align=align)
        elif kind == "icon":
            return self.addIconButton(title, func, extra, row, column, colspan, rowspan, align=align)
    ...
    def _configWidget(self, title, kind, **kwargs):
        ...
        for key in ["pad", "inpad"]:
            ...
            if val is not None:
                gui.error("Invalid argument for %s %s - %s:%s",
                          WIDGET_NAMES.name(kind), title, key, val)
        ...
        if tooltip is not None:
            self._addTooltip(widget, tooltip, None)
        if focus:
            widget.focus_set()

        if change is not None:
            self._bindEvent(kind, title, widget, change, "change", key=None)
        if submit is not None:
            self._bindEvent(kind, title, widget, submit, "submit", key=None)
        if over is not None:
            self._bindOverEvent(kind, title, widget, over, None, None)
        if drag is not None:
            self._bindDragEvent(kind, title, widget, drag, None, None)
        if drop is not None:
            self._registerExternalDropTarget(title, widget, drop)
        if right is not None:
            self._bindRightClick(widget, right)
    ...
    def _buildButton(self, title, func, frame, name=None):
        ...
        if isinstance(title, list):
            raise Exception("Can't add a button using a list of names: " +
                            str(title) + " - you should use .addButtons()")
        self.widgetManager.verify(WIDGET_NAMES.Button, title)
        if not self.ttkFlag:
...
#####################################
    ...
    def microbit(self, title, *args, **kwargs):
        ...
        if image is not None:
            self.setMicroBitImage(title, image)
        if brightness is not None:
            self.setMicroBitPixel(title, x, y, brightness)
        if clear:
            self.clearMicroBit(title)
...
#####################################
    ...
    def datePicker(self, title, value=None, *args, **kwargs):
        ...
        if value is not None:
            if toValue is None:
                self.setDatePicker(title, value)
            else:
                self.setDatePickerRange(title, startYear=value, endYear=toValue)
        if change is not None:
            self.setDatePickerChangeFunction(title, change)
...
#####################################
    ...
    def _makeAccess(self):
        if not self.accessMade:
            ...
            def _settings():
                font = {"underline": self.check("access_underline_check"),
                        "overstrike": self.check("access_overstrike_check")}
                font["weight"] = "bold" if self.check("access_bold_check") is True else "normal"
                font["slant"] = "roman" if self.radio(
                    "access_italic_radio") == "Normal" else "italic"
                if len(self.listbox("access_family_listbox")) > 0:
                    font["family"] = self.listbox("access_family_listbox")[0]
                if self.option("access_size_option") is not None:
                    font["size"] = self.option("access_size_option")

                if self.check('access_label_check'):
                    self.labelFont = font
                if self.check('access_input_check'):
                    self.inputFont = font
                if self.check('access_button_check'):
            ...
            with self.subWindow("access_access_subwindow", sticky="news", title="Accessibility", resizable=False) as sw:
                ...
                with self.labelFrame("access_font_labelframe", sticky="news", name="Font") as lf:
                    ...
                    with self.frame("access_ticks_frame", colspan=2):
                        self.check("access_label_check", True, label="Labels", pos=(
                            0, 0), font=self._accessFont, tip="Set label fonts")
                        self.check("access_input_check", label="Inputs", pos=(
                            0, 1), font=self._accessFont, tip="Set input fonts")
                        self.check("access_button_check", label="Buttons", pos=(
                            0, 2), font=self._accessFont, tip="Set button fonts")

                    self.listbox("access_family_listbox", self.fonts, rows=6,
                                 tip="Choose a font", colspan=2, font=self._accessFont)
                    self.option("access_size_option", [7, 8, 9, 10, 12, 13, 14, 16, 18, 20, 22, 25,
                                29, 34, 40], label="Size:", tip="Choose a font size", font=self._accessFont)
                    self.check("access_bold_check", name="Bold", pos=('p', 1),
                               tip="Check this to make all font bold", font=self._accessFont)
                    self.radio("access_italic_radio", "Normal",
                               tip="No italics", font=self._accessFont)
                    self.radio("access_italic_radio", "Italic", pos=('p', 1),
                               tip="Set font italic", font=self._accessFont)
                    self.check("access_underline_check", name="Underline",
                               tip="Underline all text", font=self._accessFont)
                    self.check("access_overstrike_check", name="Overstrike", pos=(
                        'p', 1), tip="Strike out all text", font=self._accessFont)
                with self.labelFrame("access_colour_labelframe", sticky="news", name="Colours") as lf:
                    if not self.ttk:
                        lf.config(padx=5, pady=5, font=self._accessFont)
                    self.label("access_fg_text", "Foreground:", sticky="ew",
                               anchor="w", font=self._accessFont)
                    self.label("access_fg_colBox", "", pos=('p', 1), sticky="ew", submit=_changeFg, relief="ridge",
                               tip="Click here to set the foreground colour", font=self._accessFont, width=14)
                    self.label("access_bg_text", "Background:", sticky="ew",
                               anchor="w", font=self._accessFont)
                    self.label("access_bg_colBox", "", pos=('p', 1), sticky="ew", submit=_changeBg, relief="ridge",
                               tip="Click here to set the background colour", font=self._accessFont, width=14)
                self.sticky = "se"
                with self.frame("access_button_box"):
                    self.button("access_apply_button", _settings, name="Apply",
                                pos=(0, 0), font=self._accessFont)
                    self.button("access_reset_button", self._resetAccess,
                                name="Reset", pos=(0, 1), font=self._accessFont)
                    self.button("access_close_button", _close, name="Close",
                                pos=(0, 2), font=self._accessFont)
    ...
    def _resetAccess(self):
        if self.accessMade:
            ...
            if self.accessOrigFont["weight"] == "normal":
                self.check("access_bold_check", False)
            else:
                self.check("access_bold_check", True)

            if self.accessOrigFont["slant"] == "roman":
                self.radio("access_italic_radio", "Normal")
            else:
                self.radio("access_italic_radio", "Italic")
...
#####################################
    ...
    def _parsePos(self, pos, kwargs):
        ...
        if len(pos) > 1:
            kwargs["column"] = pos[1]
        if len(pos) > 2:
            kwargs["colspan"] = pos[2]
        if len(pos) > 3:
            kwargs["rowspan"] = pos[3]
    ...
    def label(self, title, value=None, *args, **kwargs):
        ...
        try:
            ...
            if value is not None:
                self.setLabel(title, value)
            label = self.getLabel(title)
        else:  # new widget
            kwargs = self._parsePos(kwargs.pop("pos", []), kwargs)
            if kind == "flash":
                label = self._labelMaker(title, value, kind, *args, **kwargs)
            elif kind == "selectable":
                label = self._labelMaker(title, value, kind, *args, **kwargs)
            else:
                label = self._labelMaker(title, value, "label", *args, **kwargs)
    ...
    def _labelMaker(self, title, text=None, kind="label", row=None, column=0, colspan=0, rowspan=0, **kwargs):
        """ Internal wrapper, to hide kwargs from original add functions """
        if kind == "flash":
            return self.addFlashLabel(title, text, row, column, colspan, rowspan)
        elif kind == "selectable":
            return self.addSelectableLabel(title, text, row, column, colspan, rowspan)
        elif kind == "label":
            return self.addLabel(title, text, row, column, colspan, rowspan)
    ...
    def addLabel(self, title, text=None, row=None, column=0, colspan=0, rowspan=0, selectable=False):
        ...
        if text is None:
            gui.trace(
                "Not specifying text for labels (%s) now uses the title for the text. If you want an empty label, pass an empty string ''", title)
            text = title

        if not selectable:
            if not self.ttkFlag:
                lab = Label(self.getContainer(), text=text)
                lab.config(justify=LEFT, font=self._getContainerProperty(
                    'labelFont'), background=self._getContainerBg())
            ...
            lab.config(justify=CENTER, font=self._getContainerProperty(
                'labelFont'), background=self._getContainerBg())
            lab.origBg = self._getContainerBg()
        ...
        for i in range(len(names)):
            ...
            if not self.ttkFlag:
                ...
                lab.config(font=self._getContainerProperty('labelFont'),
                           justify=LEFT, background=self._getContainerBg())
            else:
...
#####################################
    ...
    def textArea(self, title, value=None, *args, **kwargs):
        ...
        try:
            ...
            kwargs = self._parsePos(kwargs.pop("pos", []), kwargs)
            if scroll:
                text = self._textMaker(title, "scroll", *args, **kwargs)
            else:
                text = self._textMaker(title, "text", *args, **kwargs)
            callFunction = False
        ...
        if replace:
            self.clearTextArea(title)
        if value is not None:
            self.setTextArea(title, value, end=end, callFunction=callFunction, tag=tag)
        if disabled:
            self.disableTextArea(title)
        if len(kwargs) > 0:
            self._configWidget(title, widgKind, **kwargs)
        return text
    ...
    def _textMaker(self, title, kind="text", row=None, column=0, colspan=0, rowspan=0, *args, **kwargs):
        if kind == "scroll":
            return self.addScrolledTextArea(title, row, column, colspan, rowspan)
        elif kind == "text":
            return self.addTextArea(title, row, column, colspan, rowspan)
    ...
    def addTextArea(self, title, row=None, column=0, colspan=0, rowspan=0, text=None):
        ...
        if text is not None:
            self.setTextArea(title, text, callFunction=False)
        return txt
    ...
    def textAreaToggleTagRange(self, title, tag, start, end=END):
        ...
        ta = self.widgetManager.get(WIDGET_NAMES.TextArea, title)
        if tag in ta.tag_names(start):
            self.textAreaUntagRange(title, tag, start, end)
        else:
            self.textAreaTagRange(title, tag, start, end)
...
#####################################
    ...
    def tree(self, title, value=None, *args, **kwargs):
        ...
        if click is not None:
            self.setTreeClickFunction(title, click)
        if edit is not None:
            self.setTreeEditFunction(title, edit)
        if dblClick is not None:
            self.setTreeDoubleClickFunction(title, dblClick)
        if editable is not None:
            self.setTreeEditable(title, editable)
        if showAttr is not None:
            self.showTreeAttributes(title, showAttr)
        if showMenu is not None:
            self.showTreeMenu(title, showMenu)
        return tree
...
#####################################
    ...
    def entry(self, title, value=None, *args, **kwargs):
        ...
        try:
            ...
            if value is not None:
                self.setEntry(title, value, *args, **kwargs)
            ...
            if kind == "auto":
                if value is None:
                    value = []
                ent = self._entryMaker(title, *args, secret=secret,
                                       kind=kind, words=value, **kwargs)
            else:
        ...
        if limit is not None:
            self.setEntryMaxLength(title, limit)
        if case == "upper":
            self.setEntryUpperCase(title)
        elif case == "lower":
            self.setEntryLowerCase(title)
        ...
        if kind != "auto":
            if value is not None:
                self.setEntry(title, value)
        else:
            if rows is not None:
                self.setAutoEntryNumRows(title, rows)
    ...
    def setAutoEntryNumRows(self, title, rows):
        ...
        try:
            ...
            gui.error(
                "You can only change the number of rows in an AutoEntry, %s is not an AutoEntry.", title)
...
#####################################
    ...
    def meter(self, title, value=None, *args, **kwargs):
        ...
        try:
            self.widgetManager.verify(WIDGET_NAMES.Meter, title)
        except:  # widget exists
            meter = self.getMeter(title)
        else:  # new widget
            kwargs = self._parsePos(kwargs.pop("pos", []), kwargs)
            if kind == "split":
                meter = self._addMeter(title, "SPLIT", **kwargs)
            elif kind == "dual":
                meter = self._addMeter(title, "DUAL", **kwargs)
            else:
                meter = self._addMeter(title, "METER", **kwargs)

        if value is not None:
            self.setMeter(title, value, text=text)
        if fill is not None:
            self.setMeterFill(title, fill)
...
#####################################
    def pie(self, title, value=None, *args, **kwargs):
        ...
        try:
            ...
            if name is not None:
                self.setPieChart(title, name, value)
    ...
    def toolbar(self, names, funcs, **kwargs):
        ...
        for x, n in enumerate(names):
            if icons is not None:
                try:
                    self.setToolbarIcon(n, icons[x])
                except:
                    pass
            if status is not None:
                try:
                    self.setToolbarButtonDisabled(n, not status[x])
                except:
                    pass

        if pinned is not None:
            self.setToolbarPinned(pinned=pinned)
        if disabled is not None:
            self.setToolbarDisabled(disabled=disabled)
        if hidden is True:
            self.hideToolbar()
...
#####################################
    ...
    def addMenuItem(self, title, item, func=None, kind=None, shortcut=None, underline=-1, rb_id=None, createBinding=True):
        ...
        if shortcut is not None:
            if kind == 'cb':
                def f(e): return self._menuCheckButtonBind(title, item, func)
            ...
            try:
                ...
                raise ItemLookupError('Unable to bind menu ' + item + ' to ' +
                                      binding.displayName + ' - binding already exists')
        ...
        if item == "-" or kind == "separator":
            ...
            theMenu.add_radiobutton(label=rb_id, command=func, variable=var,
                                    value=rb_id, accelerator=acc, underline=underline)
            if newRb:
            ...
            var.set(False)
            self.widgetManager.add(WIDGET_NAMES.Menu, varName, var, group=WidgetManager.VARS)
            theMenu.add_checkbutton(label=item, command=func, variable=var,
                                    onvalue=True, offvalue=False, accelerator=acc, underline=underline)
        elif kind == "sub":
    ...
    def _prepareCopyAndPasteMenu(self, event, widget=None):
        if self.copyAndPaste.inUse:
            if event is not None:
                widget = event.widget
            self._changeMenuState(self.widgetManager.get(
                WIDGET_NAMES.Menu, "EDIT"), DISABLED, 'Disabling', 10)
    ...
    def addMenu(self, name, func, shortcut=None, underline=-1):
        self.addMenuItem(None, name, func=func, kind="topLevel",
                         shortcut=shortcut, underline=underline)
    ...
    def menu(self, menu, name=None, func=None, **kwargs):
        ...
        if kind == 'menu':
            ...
            self.addMenuRadioButton(menu, group, value=name, func=func,
                                    shortcut=shortcut, underline=underline)
        elif kind == 'button':
            if name is None and func is not None:
                ...
                self.addMenuItem(menu, name, func=func, kind=None,
                                 shortcut=shortcut, underline=underline)
        ...
        if state is not None:
            if kind == 'menu' or kind.startswith('sub'):
                if state == 'disabled':
                    self.disableMenu(menu, name)
                elif state == 'enabled':
                    self.enableMenu(menu, name)
            else:
                if state == 'disabled':
                    self.disableMenuItem(menu, name)
                elif state == 'enabled':
                    self.enableMenuItem(menu, name)

        if image is not None:
            self.setMenuImage(menu, name, image, align=align)
        if icon is not None:
            self.setMenuIcon(menu, name, icon, align=align)
    ...
    def setMenuImage(self, menu, title, image, align="left"):
        ...
        try:
            ...
            gui.error("Unable to set image for menu item: %s, in menu: %s - item not found", title, menu)
    ...
    def addMenuEdit(self, inMenuBar=False):
        ...
        try:
            self.widgetManager.verify(WIDGET_NAMES.Menu, "EDIT")
        except:
            return
        ...
        eList = [
            ...
            ('Select All', lambda e: self._copyAndPasteHelper("Select All"),
             "A", True if gui.GET_PLATFORM() == gui.MAC else False),
        ...
        self.addMenuSeparator("EDIT")
        self.addMenuItem("EDIT", 'Undo', lambda e: self._copyAndPasteHelper(
            "Undo"), shortcut=shortcut + "Z", createBinding=False)
        self.addMenuItem("EDIT", 'Redo', lambda e: self._copyAndPasteHelper(
            "Redo"), shortcut=shortcut+"Shift-Z", createBinding=True)

        self.addMenuSeparator("EDIT")
        self.addMenuItem("EDIT", "Bold", lambda e: self._copyAndPasteHelper(
            "BOLD"), shortcut=shortcut+"B")
        self.addMenuItem("EDIT", "Italic", lambda e: self._copyAndPasteHelper(
            "ITALIC"), shortcut=shortcut+"I")
        self.addMenuItem("EDIT", "Underline", lambda e: self._copyAndPasteHelper(
            "UNDERLINE"), shortcut=shortcut+"U")
        self.addMenuItem("EDIT", "Bold & Italic", lambda e: self._copyAndPasteHelper(
            "BOLD_ITALIC"), shortcut=shortcut+"Shift-B")
    ...
    def statusbar(self, *args, **kwargs):
        ...
        if not self.hasStatus:
            ...
            if header is not None:
                self.setStatusbarHeader(header)
            self.setStatusbar(text=text, field=field)

        if bg is not None:
            self.setStatusbarBg(bg)
        if fg is not None:
            self.setStatusbarFg(fg)
        if width is not None:
            self.setStatusbarWidth(width)
...
#####################################
    ...
    def popUp(self, title, message=None, kind="info", parent=None):
        ...
        if kind == "info":
            return self.infoBox(title, message, parent)
        elif kind == "error":
            return self.errorBox(title, message, parent)
        elif kind == "warning":
            return self.warningBox(title, message, parent)
        elif kind == "yesno":
            return self.yesNoBox(title, message, parent)
        elif kind == "question":
            return self.questionBox(title, message, parent)
        elif kind == "ok":
            return self.okBox(title, message, parent)
        elif kind == "retry":
            return self.retryBox(title, message, parent)
        elif kind == "string":
            return self.stringBox(title, message, parent)
        elif kind == "integer":
            return self.integerBox(title, message, parent)
        elif kind == "float":
            return self.floatBox(title, message, parent)
        elif kind == "text":
            return self.textBox(title, message, parent)
        elif kind == "number":
            return self.numberBox(title, message, parent)
        else:
            gui.error("Invalid popUp kind: %s, with title: %s", kind, title)
    ...
    def openBox(self, title=None, dirName=None, fileTypes=None, asFile=False, parent=None, multiple=False, mode='r'):
        ...
        if asFile:
            options["mode"] = mode
            if multiple:
                files = list(filedialog.askopenfiles(**options))
            else:
                files = filedialog.askopenfile(**options)

            return files
        # will return "" if cancelled
        else:
            if multiple:
                files = list(self.topLevel.tk.splitlist(filedialog.askopenfilenames(**options)))
            else:
                files = filedialog.askopenfilename(**options)
...
#                super(AjTreeNode, self).config(cnf, **kw)
            ...
            def drawtext(self):
                ...
                if self.hasAttr:
                    self.attrId = self.canvas.create_text(
                        self.x+20-1, self.y-1, anchor="nw", text='*')
                    self.x += 7
            ...
            def _bindMenu(self):
                if self.canvas.menu is not None and not self.menuBound:
                    ...
                    if gui.GET_PLATFORM() in [gui.WINDOWS, gui.LINUX]:
                        ...
                        if self.hasAttr:
                            self.canvas.tag_bind(self.attrId, "<Button-3>", self._showMenu)
                        self.label.bind("<Button-3>", self._showMenu)
                    else:
                        self.canvas.tag_bind(self.image_id, "<Button-2>", self._showMenu)
                        if self.hasAttr:
                            self.canvas.tag_bind(self.attrId, "<Button-2>", self._showMenu)
    ...
    def _makeAjTreeData(self):
        ...
        # REQUIRED FUNCTIONS
            ...
            def getAttribute(self, att='id'):
                try:
                    return self.node.attributes[att].value
                except:
                    return None
    ...
    def showPage(self, page):
        try:
            ...
            raise Exception("Invalid page number: " + str(page) +
                            ". Must be between 1 and " + str(self.frameStack.getNumFrames()))
...
class TextParent(object):
    def _init(self):
        ...
        self._boldItalicFont = tkFont.Font(
...
class SelectableLabel(Entry, object):
    def __init__(self, parent, **opts):
        ...
        self.configure(relief=FLAT, state="readonly", readonlybackground='#FFFFFF',
...
class GridCell(Label, object):
    ...
    def updateFonts(self, fonts):
        self.fonts = fonts
        if self.isHeader:
            self.config(font=self.fonts["headerFont"], background=self.fonts["headerBg"],
                        fg=self.fonts['headerFg'], relief=self.fonts['border'])
        else:
            if self.selected:
                self.config(font=self.fonts["dataFont"], background=self.fonts["selectedBg"],
                            fg=self.fonts['selectedFg'], relief=self.fonts['border'])
            else:
                self.config(font=self.fonts["dataFont"], background=self.fonts["inactiveBg"],
                            fg=self.fonts['inactiveFg'], relief=self.fonts['border'])
...
class SimpleTable(ScrollPane):
    ...
    def configure(self, cnf=None, **kw):
        ...
        if "activebg" in kw or "activebackground" in kw:
            self.fonts["selectedBg"] = kw.pop("activebg", kw.pop(
                "activebackground", self.fonts['selectedBg']))
            updateCells = True
        if "activefg" in kw or "activeforeground" in kw:
            self.fonts["selectedFg"] = kw.pop("activefg", kw.pop(
                "activeforeground", self.fonts['selectedFg']))
            updateCells = True

        if "inactivebg" in kw or "inactivebackground" in kw:
            self.fonts["inactiveBg"] = kw.pop("inactivebg", kw.pop(
                "inactivebackground", self.fonts['inactiveBg']))
            updateCells = True
        if "inactivefg" in kw or "inactiveforeground" in kw:
            self.fonts["inactiveFg"] = kw.pop("inactivefg", kw.pop(
                "inactiveforeground", self.fonts['inactiveFg']))
            updateCells = True

        if "font" in kw:
            font = kw.pop("font")
            self.fonts["headerFont"].configure(family=font.actual(
                "family"), size=font.actual("size") + 2, weight="bold")
            updateCells = True

        if "buttonfont" in kw:
            buttonFont = kw.pop("buttonfont")
            self.fonts["buttonFont"].configure(family=buttonFont.actual(
                "family"), size=buttonFont.actual("size")-2)
            updateCells = True
    ...
    def addRows(self, data, scroll=True):
        ...
        try:
            gui.trace("Adding %s rows in addRows()", len(data))
        except:
            gui.trace("Adding cursor in addRows()")
            ...
            for loop in range(position+1, len(self.cells)-1):
                ...
                for but in self.rightColumn[loop].but:
                    if butCount == 1:
                        ...
                        command = lambda name=but.cget['text'], row=loop, * \
                            args: self.action(name, row)
    ...
    def _addRow(self, rowData):
        if self.numColumns == 0:
            ...
            if numCols > self.numColumns:
                gui.warn(
                    "New data has more columns (%s) than the table (%s), some columns will be discarded.", numCols, self.numColumns)
            ...
            if self.action is not None:
                ...
                if rowNum == 0:
                    widg = GridCell(self.interior, self.fonts,
                                    isHeader=True, text=self.actionHeading)
                # add a button
                else:
    ...
    def _buildMenu(self):
        ...
        self.menu.add_command(label="Delete Column",
                              command=lambda: self._menuHelper("deleteColumn"))
        self.menu.add_command(label="Delete Row", command=lambda: self._menuHelper("deleteRow"))
        self.menu.add_separator()
        self.menu.add_command(label="Sort Ascending",
                              command=lambda: self._menuHelper("sortAscending"))
        self.menu.add_command(label="Sort Descending",
                              command=lambda: self._menuHelper("sortDescending"))
        self.menu.add_separator()
        self.menu.add_command(label="Insert Before",
                              command=lambda: self._menuHelper("columnBefore"))
        ...
        self.menu.add_command(label="Select Column",
                              command=lambda: self._menuHelper("selectColumn"))
    ...
    def _menuHelper(self, action):
        ...
        if action == "sortDescending":
            ...
            self.newText = None
            try:
                self.newText = self.clipboard_get()
            except:
                pass
            self._updateCell()
        elif action == "clear":
            ...
            defaultVar.set(val)
            self.newText = TextDialog(self, "Edit", "Enter the new text",
                                      defaultVar=defaultVar).result
            self._updateCell()
        ...
        if columnNumber < 0 or columnNumber > self.numColumns:
            ...
            if self.action is not None:
                for rowPos in range(cellCount):
                    ...
                    self.rightColumn[rowPos].grid(
                        row=rowPos, column=self.numColumns+1, sticky=N+E+S+W)
    ...
    def _createEntryBox(self, cellNum):
        ...
        # create the entry
        ent = Entry(lab, relief=FLAT, borderwidth=1, highlightbackground='black',
                    highlightthickness=1, width=6, disabledbackground='grey')
...
class MicroBitSimulator(Frame, object):
    ...
    COLOURS = {0: '#000000', 1: '#110000', 2: '#220000', 3: '#440000', 4: '#660000',
               5: '#880000', 6: '#aa0000', 7: '#cc0000', 8: '#ee0000', 9: '#ff0000'}
    SIZE = 5
...
#        self.LOCATION_URL = "http://ipinfo.io/json"
        ...
        self.buttons[0].bind("<ButtonRelease-1>",
                             lambda e: self.buttons[0].config(relief=GROOVE), add="+")
        ...
        self.buttons[1].bind("<ButtonRelease-1>",
                             lambda e: self.buttons[1].config(relief=GROOVE), add="+")
        ...
        self.buttons[2].bind("<ButtonRelease-1>",
                             lambda e: self.buttons[2].config(relief=GROOVE), add="+")
...
#        self.params["mobile"] = "true" # optional: mobile=true will assume the image is shown on a small screen (mobile device)
    ...
    def addMarker(self, location, size=None, colour=None, label=None, replace=False):
        ...
        if len(self.markers) == 0 or not replace:
            self.markers.append({"location": location, "size": size,
                                "colour": colour, "label": label})
        else:
            self.markers[-1] = {"location": location,
                                "size": size, "colour": colour, "label": label}
        ...
        if len(self.markers) > 0:
            ...
            for mark in self.markers:
                if mark["colour"] is not None:
                    m += "color:" + str(mark["colour"])
                if mark["size"] is not None:
                    m += "|size:" + str(mark["size"])
                if mark["label"] is not None:
                    m += "|label:" + str(mark["label"])
                m += "|" + str(mark["location"])
                m = quote_plus(m)
...
class DraggableWidget(object):
    ...
    def dnd_end(self, target, event):
        ...
        # have been dropped somewhere
            ...
            gui.trace("<<DRAGGABLE_WIDGET.dnd_end>> dropped with Canvas(%s) Target=%s",
                      self.Canvas, self.dropTarget)
...
#            self.dropTarget = None
        ...
        if self.Canvas:
            ...
            self.ID = self.Canvas.create_window(
                self.X-self.OffsetX, self.Y-self.OffsetY, window=self.Label, anchor="nw")
...
class WidgetManager(object):
    ...
    def group(self, widgetType, group=None, array=False):
        ...
        """
        if group is None:
            container = self.widgets
        elif group == WidgetManager.VARS:
            container = self.vars
    ...
    def log(self, widgetType, widget, group=None):
        ...
        try:
            ...
            pass  # not available on some classes
    ...
    def check(self, widgetType, widgetName, group=None):
        ...
        try:
            if widgetName in self.group(widgetType, group):
                return True
            else:
                raise ItemLookupError("Invalid widgetName: '" + widgetName)
        except KeyError:
    ...
    def remove(self, widgetType, widgetName, group=None):
        ...
        if type(widgGroup) == list:
            ...
            if group != self.VARS:
                try:
                    del self.group(widgetType, self.VARS)[widgetName]
                except:
                    pass
